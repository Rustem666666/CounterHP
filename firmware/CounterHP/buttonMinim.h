...
void buttonMinim::tick() {
  ...
  if (!btnState && !flags.btnFlag && ((uint32_t)millis() - _btnTimer > 10)) {
    flags.btnFlag = true;
    _btnTimer = millis();
    flags.pressF = true;
    flags.holdedFlag = true;
  }
  if (btnState && flags.btnFlag && ((uint32_t)millis() - _btnTimer < 50)) {
    flags.btnFlag = false;
    _btnTimer = millis();
    flags.clickF = true;
    flags.holdF = false;
  }
  if (flags.btnFlag && ((uint32_t)millis() - _btnTimer > 80)) {
    if (!btnState) {
      flags.holdF = true;
    } else {
      flags.btnFlag = false;
